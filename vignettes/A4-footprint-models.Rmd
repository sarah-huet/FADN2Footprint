...
at the individual product level. Following the JRC ILCD handbook (JRC, 2010), we avoid economic allocation wherever
...
Fertilizers and pesticides are virtually distributed to each crop through a national allocation 
key based on the crop practices survey. See XX for details.

### Off-road diesel
Off-road diesel use was allocated in two steps. 
First, diesel consumption for ploughing was estimated at crop level using tillage intensity in L/ha (see `f_tillage()`) multiplied by crop area, with crops not subject to tillage assigned zero tillage diesel. 
Second, the remaining farm-level diesel consumption, after subtracting tillage diesel, was economically allocated across crops and livestock outputs. Crop-level diesel emissions hence sum up the tillage-related and residual components.
Diesel consumption values are then multiplied by a fixed emission factor retrieve from the UNFCCC CRF tables (see `f_ghg_diesel()`).

### Heating fuel

We consider that heating fuels are mostly used for heating greenhouses and livestock buildings. Indeed, in our test dataset, more than 80% of crop specialists do not consume any heating fuel.  
Thus, we economically allocated heating fuel emissions between greenhouse and livestock outputs. Nonetheless, if a farm do not have any greenhouse or livestock but do consume heating fuel, we economically allocate heating fuels to crop outputs.

$$ghg_{heating fuel,o} = ghg_{heating fuel,farm} \times econ\_alloc_o$$

### Electricity emissions
Electricity emissions are economically allocated
to individual products:

$$econ\_alloc_p = \frac{TO_p}{\sum_o TO_p}$$

where $TO_p$ is the economic output of product $p$. The allocated electricity
emissions for each product were then:
...
cull cow meat from dairy herd) [van der Werf et al., 2009, JRC - IES, 2010].
The total herd impact is the sum of: (i) the impact of all feed consumed (on-farm grown and purchased), (ii) direct on-farm emissions from animals (e.g., manure management, enteric fermentation), and (iii) energy consumption allocated to the products.
...
indicators computed by the package.
...
Rows represent individual emission variables, 
organized into four groups: **crop emissions** (N₂O and CO₂ from fertilizer 
production, diesel, and electricity), **herd feed emissions from on-farm crops**, 
**herd feed emissions from pseudofarm**, and **direct livestock 
emissions** (enteric CH₄, manure CH₄, and manure N₂O). Columns represent aggregate 

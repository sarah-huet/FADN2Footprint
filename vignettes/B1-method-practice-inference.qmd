...
|                | keys)          |                | other sources  |
+----------------+----------------+----------------+----------------+
| Crop practices | N fertilizer   | Hedge density  |                |
|                | quantity,      |                |                |
...
|                                  |                                  |
|                                  | Animal product production (milk, |
|                                  | meat, ...)                       |
+----------------------------------+----------------------------------+
...
-   The farm has the same PACAGE number in FADN: then the LPIS plots
    with this PACAGE number are associated with that FADN-AC-FQS farm;
...
-   No PACAGE number is registered in the FADN , or the PACAGE number in
    ...
    associated the FADN holding. In 2020, such cases are found for 443
    and 318 metropolitan farms, respectively. These holdings are mainly
...
from the equilibrium quantities estimated for fattening animals ( \* in
Table x). Conversely, we add the number of calves sold for slaughter to
equilibrium quantities of calves estimated from the observed fattening
animals ( \*\* in Table x).
...
$$
\bar{Q}_{s} = \sum^{L_s}_{l=1} \bar{Q}_{l}
$$ {#eq-Q_s_avg}
...
$$
\widehat{Q}_{l} = \frac{\widehat{Q}_{s} - \bar{Q}_s}{L_s} + \bar{Q}_l
$$ {#eq-Q_l_eq}

\<--! XXIci attribution du nb de tête à l'équilibre = division simple

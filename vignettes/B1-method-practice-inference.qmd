...
where $F$ are flows $in$ or $out$ the considered livestock category $l$ ((encompassing all animals entering or leaving the category; upward category $l-1$), L is the ensemble of categories directly downward to
...
where $l_F$ are fattening animals and $l_B$ are breeders in category $l$.
...
First, we estimate animals involved in each activity (i.e.,
milk, eggs, and meat). 

For cattle, we first estimate animals involved in 
the dairy herd with Equation @eq-restrain_herd_act. We consider all 
additional cattle as part of the meat activity (e.g. additional calves
spawned from dairy cows).
...
For swine, we consider all animals as involved in the meat activity.
For poultry, we consider that all animals needed in the production of
either eggs (i.e., laying hens) or meat (i.e., broilers and other
poultry) are present on-farm, the pseudo-herd thus equaling the herd.
...
IPCC/UNFCCC MCF and Bo parameters to compute CH4 and
(together with N excretion and EF) N2O from manure management.

## Animal weight estimation {#sec-method-practices-animal-weight}

Animal weights are not reported in FADN. We use the national animal liveweights
reported in UNFCCC CRF tables.

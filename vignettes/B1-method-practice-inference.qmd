...
|               |               |               |               |
+===============+===============+===============+===============+
| Data source / | FADN (+ Farm  | \+ LPIS & SWF | Parameters    |
| Practices     | practices     | & SENTINEL-2  | from national |
|               | survey & feed | (farm level)  | GHGIs (yearly |
|               | tables for    |               | national      |
|               | allocation    |               | averages) and |
|               | keys)         |               | other sources |
+---------------+---------------+---------------+---------------+
| Crop          | N fertilizer  | Hedge density |               |
| practices     | quantity,     |               |               |
|               | organic and   | Mean Field    |               |
|               | mineral       | Size          |               |
|               |               |               |               |
|               | Pesticides    | Ground Cover  |               |
|               |               |               |               |
|               | Tillage       |               |               |
|               |               |               |               |
|               | Crop          |               |               |
...
|               | Yields        |               |               |
+---------------+---------------+---------------+---------------+
| Grassland     | N Fertilizer  | Hedge density | Yield (XX     |
| practices     | quantity,     |               | source to     |
|               | organic and   |               | define)       |
|               | mineral       |               |               |
|               |               |               |               |
|               | Livestock     |               |               |
...
+=================================+=================================+
| Mineral nitrogen quantity       | Crop areas (ha)                 |
| (kgN/ha)                        |                                 |
|                                 | Amount of mineral nitrogen      |
|                                 | fertilizer (kgN)                |
+---------------------------------+---------------------------------+
| Organic nitrogen quantity       | Crop areas (ha)                 |
| (kgN/ha)                        |                                 |
|                                 | Feed intake (see below)         |
|                                 |                                 |
|                                 | Animal number and category      |
+---------------------------------+---------------------------------+
| Pesticides (€/ha)               | Crop areas (ha)                 |
|                                 |                                 |
|                                 | Pesticides purchases (€)        |
+---------------------------------+---------------------------------+
| Tillage (L/ha)                  | Crop areas (ha)                 |
|                                 |                                 |
|                                 | Off-road diesel purchases (L)   |
+---------------------------------+---------------------------------+
| Crop diversity (Shannon index,  | Crop areas (ha)                 |
| no unit)                        |                                 |
+---------------------------------+---------------------------------+
| Crop Yield (tDM/ha)             | Crop areas (ha) and production  |
|                                 | (tDM)                           |
+---------------------------------+---------------------------------+
| Livestock density (LU/ha)       | Grassland and feedcrops areas   |
|                                 | (ha)                            |
|                                 |                                 |
|                                 | Animal number and category      |
+---------------------------------+---------------------------------+
| Feed intake (tDM/LU/yr)         | Crop production and\            |
|                                 | sales                           |
|                                 |                                 |
|                                 | Animal number and category      |
|                                 |                                 |
|                                 | Feed purchases                  |
+---------------------------------+---------------------------------+
| Rearing parameters (yr)         | Animal number and category      |
|                                 |                                 |
|                                 | Animal sales and purchases      |
+---------------------------------+---------------------------------+
| Animal yield                    | Animal number and category      |
|                                 |                                 |
...
which practice direct seeding, i.e., cultivate crops without tillage (XX
L/ha, Chenu and Butault, 2015). The implicit assumption is that the
average off-road diesel use in these farms broadly corresponds to the
amount of diesel used for other interventions than tillage. Note that
tillage generally represents *43*% of off-road diesel consumption due to
...
Tillage intensity cannot be negative so it is set to zero if off-road
diesel consumption is lower than the reference value for direct seeding.
...
allocated to the different crops using the French average mineral
nitrogen input per crop as a distribution key (Ministère De
L’Agriculture - SSP, 2019) (see “PKGC_N_ferti” sheet of Appendix C in
Huet et al., 2026).
...
For farms without livestock, a standard value, equal to the French
average organic nitrogen input for farms that do not produce organic
manure (XX kgN/ha, Ministère De L’Agriculture - SSP, 2019; see
“PKGC_N_ferti” sheet of Appendix C in Huet et al., 2026), is added to
the different crops, distinguishing between organic and conventional
farms.
...
“PKGC_N_ferti_org” sheet in Appendix C in Huet et al., 2026).

If the resulting amount is smaller than the lowest value between i) the
French average organic nitrogen input for farms that do not produce
organic manure and ii) the lower bound of the 95% confidence interval of
the average of organic nitrogen inputs of farms producing their own
organic manure (XX kgN/ha, Ministère De L’Agriculture - SSP, 2019), we
assign the lowest of these two values, that is XX kgN/ha, instead
(implicitly concluding that despite having a few animals, these farms
import manure).
...
to crops is used as an input variable for the biodiversity model
(Lindner et al., 2019).
...
toxicity of products used in conventional agriculture, estimated as
follows.
...
“A.5.1_substance_top10” sheet of Appendix C in Huet et al., 2026). As a
result, we estimate that each euro spent on plant protection in organic
farms is 3.82 times less toxic than a plant protection euro in
conventional farms.
...
estimating these parameters, we first extract from the French LPIS (IGN,
2020) all plots from farms registered our FADN database for the same
year.
...
primary join key. We face two situations:

- The farm has the same PACAGE number in FADN: then the LPIS plots with
  this PACAGE number are associated with that FADN-AC-FQS farm;

- No PACAGE number is registered in the FADN , or the PACAGE number in
  FADN does not correspond to a LPIS PACAGE number: no plot is
  associated the FADN holding. In 2020, such cases are found for 443 and
  318 metropolitan farms, respectively. These holdings are mainly
  concentrated in the OTEX viticulture, market gardening and pig and/or
  poultry farming.
...
Firstly, we expand the LPIS blocks. Hedges are not always inside,
outside or at the edge of LPIS blocks (all three cases present).
However, they rarely cut a block in half. The first step is to expand
the LPIS blocks by a buffer zone of 10 m. The length of 10 m for the
buffer zone is determined visually. It seems that beyond 10 m, it is
common to make mistakes when combining a hedge with two blocks which are
actually separated by a road. Conversely, below 10 m, it seems common to
fail to associate a hedge to a field which is clearly its direct
neighbour.

Secondly, we intersect the hedge lines with the expanded blocks to
determine a border length. There are four cases for each piece of hedge
from the intersection:

1.  The piece of hedge does not intersect any enlarged block. The piece
    of hedge is considered to be lined with non-agricultural uses on
    both sides and twice its length is assigned to “No crop”.

2.  The piece of hedge intersects a single expanded block. The piece of
    hedge is considered to be bordered by agricultural use on one side,
    and non-agricultural use on the other. Its length is assigned once
    to the crop/grassland type of the block and another time to “No
    crop”.

3.  The hedge piece intersects two blocks. The hedge piece is considered
    to be bordered on both sides by agricultural use. Its length is
    assigned once to the type of crop/grassland of each block.

4.  The piece of hedge intersects more than two blocks. Its length could
    then be affected more than twice, which would be absurd (hedge with
    more than two sides). To avoid this, the corresponding lengths are
    ...
    equal to twice the length of the actual hedge (two sides).
...
$$H' = - \sum_{i=1}^{R} p_i ln p_i$$ {#eq-shannon}
...
For herds, we subdivide animals, feed and waste to each farm activity
(e.g. milk vs specialized meat) as recommended (ILCD, 2010). However, we
cannot use subdivision for products coming from the same livestock
category (ie., milk and cull meat from dairy cows). When subdivision is
not possible, we allocate the environmental footprint to the different
co-products using an economic allocation.
...
::: {#fig-herd-flows}
![Cattle](FADN_prod_flow_cattle.png) ![Swine](FADN_prod_flow_swine.png)
...
$$F_{l-1}^{out} = Q_{l-1}^{sales} + \sum_L (F_{l}^{in}  - Q_{l}^{purchases})$$
{#eq-Fout}

where $F$ are flows $in$ or $out$ the considered livestock category $l$
((encompassing all animals entering or leaving the category; upward
category $l-1$), L is the ensemble of categories directly downward to
$l-1$ and $Q$ are registered numbers of animals (i.e., $average$,
$opening$, $closing$, $purchases$, or $sales$).

Note that some farm can experience animal losses, resulting in negative
flows. Losses are not recorded, so they are generally assumed to be
null. The exception is when this assumption leads to negative inflows:
this is impossible, so inflow is then set to zero.
...
the time spent in that category) following @eq-rearing_time, similar to
residence time in fluid mechanics, estimated as the amount of fluid
stored on the fluid flow rate.

$$t_{l} = \frac{\bar{Q}_{l}}{\frac{F_l^{in}+F_l^{out}}{2}}$$
{#eq-rearing_time}

where $t$ is the average time an animal spends in the livestock category
$l$ (in years), $Q_{l}$ is the observed animal stock (see @eq-Qobs) in
the farm and $F_{l}$ are in- and outflows.
...
For cattle, we also estimate the average age at first calving as the
average of the minimal age (i.e., 1 or 2 y.o.) plus residence time in
...
parameter as the median farm in the same NUTS2, or, if less than 3 farms
are registered for the NUTS2, the median farm in the same NUTS1. We also
estimate thresholds below and above which a value is considered aberrant
by graphically determining outliers on the rearing parameter
distribution in data set. Note that the maximum threshold for categories
whose age is specified is logically determined by this definition (e.g.,
female bovine between 1 and 2 y.o. have a maximum residence time of 1).
When a value is below or above these thresholds, we replace the aberrant
value by the inferior or superior threshold, respectively.

#### Subdivision: estimation in mixed categories {#sec-method-practices-livestock-mixed-cat}
...
the number of animals sold are differentiated between sales for
slaughtering and those for rearing. We consider that sales for rearing
are for breeding purposes and that sales for slaughter are fattening
animals (i.e., $Q_{B}^{sales}$ and $Q_{F}^{sales}$, respectively; see
section Animal products). For other numbers (i.e., observed, average,
opening, closing, and purchases) in these mixed categories, we
distinguish animals going to be fattened from those going to be breeders
based on their proportion in the downward flow of animals (see Eq.
@eq-mixed_cat for an example calculation of the observed number of
animals for fattening).

$$\bar{Q}_{l_F} = \bar{Q}_{l} \cdot \frac{\sum (\bar{Q}_{l_F+1} \cdot \frac{1}{t_{l_F+1}})}{ \sum (\bar{Q}_{l_F+1} \cdot \frac{1}{t_{l_F+1}}) + \sum (\bar{Q}_{l_B+1} \cdot \frac{1}{t_{l_B+1}}) }$$
{#eq-mixed_cat}

where $l_F$ are fattening animals and $l_B$ are breeders in category
$l$.
...
produce veal or raised longer to produce beef. Similarly, beef comes
from bovine animals that were once calves spawned from suckler cows: the
beef impact should hence encompass the slaughtered bovine impact as well
as the calve and suckler cow ones, even when these are not present on
the same farm. The pseudo-herd accounts for all animals involved in a
...
First, we estimate animals involved in each activity (i.e., milk, eggs,
and meat).

For cattle, we first estimate animals involved in the dairy herd with
Equation @eq-restrain_herd_act. We consider all additional cattle as
...
For swine, we consider all animals as involved in the meat activity. For
poultry, we consider that all animals needed in the production of either
eggs (i.e., laying hens) or meat (i.e., broilers and other poultry) are
present on-farm, the pseudo-herd thus equaling the herd. For sheep, we
assume that a ewe produce 255 l/year in France (CAP2ER, 2025) and thus
estimate the number of ewes involved in the milk activity as
$Qobs_{milk} = prod_t / 0.255$ (where $prod_t$ is the milk production in
tonnes). We assign at least one ewe to the milk activity if the farm has
any milk production \< 255 L. All other sheep are assigned to the meat
activity. For goats, we assign all animals to the milk activity.

Second, we estimate the pseudo herd by calculating the balanced number
of animals (see next section).

#### Balanced number of animals
...
fattening or breeders), and the rearing parameters estimated XXreplace
reference to relevant sectionXX. For cattle specifically, such
estimation of the equilibrium quantities assumes that all calves reach
the next rearing stage. However, some calves are sold for slaughter and
produce veal, hence never reach downward stages. To account for
slaughtered calves, we remove the number of calves sold for slaughter
from the equilibrium quantities estimated for fattening animals
(\@tbl-pseudo#tbl-pseudoherd_eqherd_eq). Conversely, we add the number
of calves sold for slaughter to equilibrium quantities of calves
estimated from the observed fattening animals
(\@tbl-pseudo#tbl-pseudoherd_eqherd_eq).
...
encompassing $L_s$ livestock categories $l$ (e.g., bovine male above 1
...
$$\bar{Q}_{s} = \sum^{L_s}_{l=1} \bar{Q}_{l}$$ {#eq-Q_s_avg}

$$t_s = \frac{\sum^{L_s}_{l=1} (age^{start} + t_l) \cdot  \bar{Q}_{l}}{\sum^{L_s}_{l=1} \bar{Q}_{l}}$$
{#eq-t_s}

The equilibrium number of animals in each category $l$ is then obtained by distributing the residual gap between the stage-level equilibrium total $Q_s$ and the sum of observed numbers, weighted by the residence time of each category:

$$Q_l = Q_l^{obs} + \left(Q_s - \sum_{l=1}^{L_s} Q_l^{obs}\right)\cdot \frac{t_l}{\displaystyle\sum_{l=1}^{L_s} t_l}$${#eq-Q_l_eq}
...
category $l$ and rearing stage $s$ (with $L_s$ livestock categories).
...
and summing impacts of all animals. The characteristics (amount of feed
intake, residence time, yield) of the animals added to the two minority
categories are set to the NUTS2 average (or NUTS 1 if the NUTS2 has
fewer than three farms where this animal type is present). XX to be
checked but something must be explained on this aspect

For dairy cattle specifically, we estimate equilibrium quantities from
dairy cows ($\bar{Q}_{b}$). Then, we remove the number of animals needed
to renew dairy cows from the estimated equilibrium quantities
($\widehat{Q}_{j}$ and $\widehat{Q}_{f}$) and assign all other animals
to the meat activity.
...
To estimate the quantity and quality (type of) of the livestock feed, we
consider both on-farm (i.e., farm-grown) and off-farm (i.e., purchased)
feed to form a "pseudo-farm". For total quantities, we rely on produced
amounts, sold amounts and purchased values. For their allocation to each
animal category, we rely on a theoretical feed ration constructed for
each farm’s livestock category. Theoretical rations are determined by
...
al., 2021; see “Sailley_2021_feed_flows” sheet of Appendix C in Huet et
al., 2026).
...
the NUTS3 average for the corresponding year (Agreste, 2020). Purchased
feed quantity is estimated based on FADN expenditures variables for
concentrated feedstuffs and coarse fodder. We first value the
theoretical ration using feed prices (Eurostat, 2022) to determine the
theoretical cost share of each feedstuff per livestock species
(concentrates and roughage). The observed FADN expenditure is then
allocated to specific feed types according to these theoretical value
shares. Finally, these allocated values are converted into physical
quantities (tonnes) by dividing by the corresponding Eurostat unit
prices.

This net available on-farm biomass is then allocated to the farm's
livestock categories proportional to their theoretical dry matter
requirements for each crop, concentrated feedstuffs and coarse fodder.
...
tables (Tran, 2002) to the estimated dry matter quantities. To exclude
aberrant estimations, we remove farms with extreme GE supplied to their
main livestock category. Specifically, we exclude farms with a GE
exceeding the mean + 3 × standard deviation (a common threshold in the
...
non-dairy cow livestock categories (Table A4 in Huet et al., 2026).
...
AWMS fractions are inferred from UNFCCC CRF tables together with
IPCC/UNFCCC MCF and Bo parameters to compute CH4 and (together with N
excretion and EF) N2O from manure management.
...
liveweights reported in UNFCCC CRF tables.

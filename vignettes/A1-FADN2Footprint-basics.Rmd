...
Bellassen (in prep.) XX réduire la taille du logo
...
-   Environmental footprint: greenhouse gas emissions,
    biodiversity impacts (upcoming: water consumption/pollution)

-   Economic performance (price premium, value-added per annual working unit,
    operating margin per non-salaried AWU, return on assets)

-   Social performance (employment, generational renewal, farmer education)
...
1.  Prepare your data (and, if you are using national FADN data),
    your variable dictionary to map your column names to the package's
    standard. This variable dictionary is provided for the French FADN.XX indiquer comment on y accède
...
If your data already uses the variable names of FADN 2018 (or if you rely on version
inference via `var_dict` for earlier FADN vintagesXX c'est ça que tu veux dire?), you can build the object directly.
XX moi je fournirai plutôt le mock data sous format excel, comme tu le fais pour les tables de conversion en A2. C'est ce que les utilisateurs auront,
donc ça leur permet de s'y retrouver. J'y laisserai notamment la description des variables, et je limiterais aux variables effectivement utilisées par le package.
...
# See Huet, S., Diallo, A., Regolo, J., Ihasusta, A., Arnaud, L., Bellassen, V., 2026. Biodiversity impact of agricultural products in France: Greater differences across product types than food quality schemes. Ecological Economics. XX doi

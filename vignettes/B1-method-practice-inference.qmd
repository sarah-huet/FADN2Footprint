...
$$
F_{l}^{in} = \Delta \text{Stock}_{l} + F_{l}^{out}
$$
...
$$
F_{l-1}^{out} = Q_{l-1}^{sales} + \sum_L F_{l}^{in}
$$ {#eq-Fout}

where $F$ are flows $in$ or $out$ the considered livestock category $l$
(or upward category $l-1$), L is the ensemble of categories downward to
...
this is impossible, so inflow is then set to zero and the negative .
...
average of the age plus residence time in
...
$$
\bar{Q}_{F} = \bar{Q}_{FB} \cdot \frac{\sum_{L_F} F_{l+1}^{in} }{\sum_{L_{FB}} F_{l+1}^{in}} \\
= \bar{Q}_{FB} \cdot \frac{\sum (\bar{Q}_{F+1} \cdot \frac{1}{t_{F+1}})}{ \sum (\bar{Q}_{F+1} \cdot \frac{1}{t_{F+1}}) + \sum (\bar{Q}_{B+1} \cdot \frac{1}{t_{B+1}}) }
$$

where $L_F$ is the ensemble of fattening animal categories at level
$l+1$ and $L_FB$ is the ensemble of all animal categories at level
$l+1$.

XX pour moi la partie suivante est redondante avec ce qui précèdeXX
...
First, we estimate animals involved in each activity (milk and meat)
with Equation @eq-restrain_herd_act.
...
climate‑region) AWMS allocations reported in UNFCCC CRF tables XX is it
worth it? if so, explainXX. Those inferred AWMS fractions are then used
together with IPCC/UNFCCC MCF and Bo parameters to compute CH4 and
